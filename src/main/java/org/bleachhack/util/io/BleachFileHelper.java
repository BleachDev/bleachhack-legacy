...
public class BleachFileHelper {
	...
	public static void saveModules() {
		...
		for (Module mod : ModuleManager.getModules()) {
			...
			if (setjson.entrySet().size() != 0)
				modjson.add("settings", setjson);

			if (modjson.entrySet().size() != 0)
	...
	public static void saveUI() {
		...
		for (Entry<String, UIWindow> w : UIClickGuiScreen.INSTANCE.getUIContainer().windows.entrySet()) {
			...
			for (Entry<String, Integer> atm: w.getValue().position.getAttachments().entrySet()) {
				ja.add(atm.getKey(), new JsonPrimitive(atm.getValue()));
			}

			if (ja.entrySet().size() > 0) {
