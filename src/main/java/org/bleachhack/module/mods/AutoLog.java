...
public class AutoLog extends Module {
	...
	private String getLogText() {
		int playerHealth = (int) (mc.field_3805.getHealth() + mc.field_3805.getAbsorption());

		if (getSetting(0).asToggle().getState()) {
			int health = getSetting(0).asToggle().getChild(0).asSlider().getValueInt();

			if (playerHealth <= health) {
				return "[AutoLog] Your health (" + playerHealth + " HP) was lower than " + health + " HP.";
			}

			if (getSetting(0).asToggle().getChild(1).asToggle().getState() &&  mc.field_3805.vehicle instanceof LivingEntity) {
				LivingEntity vehicle = (LivingEntity) mc.field_3805.vehicle;
				int vehicleHealth = (int) (vehicle.getHealth() + vehicle.getAbsorption());

				if (vehicleHealth < health) {
					return "[AutoLog] Your vehicle health (" + vehicleHealth + " HP) was lower than " + health + " HP.";
				}
			}
		}

		if (getSetting(1).asToggle().getState()) {
			for (PlayerEntity player: (List<PlayerEntity>) mc.world.playerEntities) {
				if ((!getSetting(1).asToggle().getChild(0).asToggle().getState() && BleachHack.friendMang.has(player))
						|| player == mc.field_3805) {
					continue;
				}
		...
		if (getSetting(2).asToggle().getState()) {
			for (Entity e: (List<Entity>) mc.world.getLoadedEntities()) {
				if (e instanceof EndCrystalEntity && mc.field_3805.distanceTo(e) <= getSetting(2).asToggle().getChild(0).asSlider().getValue()) {
					return "[AutoLog] End crystal appeared within range.";
				}
			}
		}
	...
	private void log(String reason) {
		mc.field_3805.field_1667.method_1205().disconnect(reason);
