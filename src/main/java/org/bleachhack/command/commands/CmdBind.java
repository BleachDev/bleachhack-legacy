			...
			for (Module m : ModuleManager.getModules()) {
				if (m.getName().equalsIgnoreCase(args[1])) {
					if (args[0].equalsIgnoreCase("set")) {
						...
						try {
							key = Keyboard.getKeyIndex(args[2].toUpperCase(Locale.ENGLISH));
						} catch (IllegalArgumentException e) {
							if (args[2].toLowerCase(Locale.ENGLISH).startsWith("right")) {
								try {
									key = Keyboard.getKeyIndex(args[2].toUpperCase(Locale.ENGLISH).replace("RIGHT_", "R").replace("RIGHT", "R"));
								...
								try {
									key = Keyboard.getKeyIndex(args[2].toUpperCase(Locale.ENGLISH).replace("LEFT_", "L").replace("LEFT", "L"));
								} catch (IllegalArgumentException e1) {
