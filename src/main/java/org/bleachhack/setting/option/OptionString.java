	...
	public WindowWidget getWidget(int x, int y, int width, int height) {
		return new WindowTextFieldWidget(x + 1, y + 1, width - 2, height - 2, "") {
			...
			if (validator != null && !validator.apply(getRealValue())) {
				DrawableHelper.fill(wx + w.x1 - 1, wy + w.y1 - 1, wx + w.x2 + 1, wy + w.y1, 0xffd07070);
				DrawableHelper.fill(wx + w.x1 - 1, wy + w.y2, wx + w.x2 + 1, wy + w.y2 + 1, 0xffd07070);
				DrawableHelper.fill(wx + w.x1 - 1, wy + w.y1, wx + w.x1, wy + w.y2, 0xffd07070);
				DrawableHelper.fill(wx + w.x2, wy + w.y1, wx + w.x2 + 1, wy + w.y2, 0xffd07070);
			}
		});
	}
