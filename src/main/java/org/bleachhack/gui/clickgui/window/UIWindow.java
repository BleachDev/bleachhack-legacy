...
public class UIWindow extends ClickGuiWindow {
	...
	private BiConsumer<Integer, Integer> renderConsumer;

	private UIContainer parentContainer;

	public UIWindow(Position pos, UIContainer parentContainer, BooleanSupplier enabledSupplier, Supplier<int[]> sizeSupplier, BiConsumer<Integer, Integer> renderConsumer) {
		super(0, 0, 0, 0, "", null);
	...
	public void render(int mouseX, int mouseY) {
		...
		if (dragging) {
			...
			int width = (int) _window.getScaledWidth();
			int height = (int) _window.getScaledHeight();
			int wWidth = x2 - x1;
			int wHeight = y2 - y1;
	...
	protected void drawBackground(int mouseX, int mouseY, TextRenderer textRend) {
		...
		DrawableHelper.fill(x1, y1, x2, y2, 0x90606090);
	}
	...
	public static class Position {
		...
		private Map<String, Integer> attachments = new HashMap<>(2);
		...
		public boolean addAttachment(String atmName, int atmSide) {
			...
			if (attachments.size() == 1) {
				int side = attachments.values().iterator().next();
